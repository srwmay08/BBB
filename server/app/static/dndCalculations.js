const DNDCalculations = {
    ...
    calculateCharacterDPR(pc, targetAC, targetSaves) {
        // Single, unified actions array for the whole function
        const actions = [];
        ...
        weapons.forEach(weapon => {
            ...
            if (D > 0) {
                ...
                const H = Math.max(0.05, Math.min(0.95, 1 - ((M - A) / 20)));
                const dpr_normal = (C * D) + (H * (D + B));
                const HA = Math.max(0.0025, Math.min(0.9975, 1 - Math.pow(((M - A) / 20), 2)));
                const CA = 1 - Math.pow((1 - C), 2);
                const dpr_adv = (CA * D) + (HA * (D + B));
                const HD = Math.max(0.0025, Math.min(0.9975, Math.pow(((21 - (M - A)) / 20), 2)));
